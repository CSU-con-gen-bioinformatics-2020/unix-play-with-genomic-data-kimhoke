...
```
drwxr-xr-x  16 kim  staff     512 Jan 28 21:10 .
drwxr-xr-x   8 kim  staff     256 Jan 28 21:10 ..
drwxr-xr-x   4 kim  staff     128 Jan 28 21:10 .Rproj.user
drwxr-xr-x  12 kim  staff     384 Jan 28 21:02 .git
-rw-r--r--   1 kim  staff     126 Jan 28 21:00 .gitignore
```
...
```
I did 10 not counting the ls part.
```
...
```
To make sure you are getting path correct so you don't accidentally remove the wrong things.
```

# If you had just typed `rm -r bad directory name with spaces` what would Unix have tried to remove?

```
Directories names bad, directory, name, with, and spaces.
```
...
```
They have the same names except the /1 and /2 at the end.
```
...
# Note that gzcat will decompress all files you give it and send the output to _stdout_.  Knowing this, count how many lines are in all the `R1` files in `fastq` (after decompressing them into text files), and also count up all the lines in the `R2` files in `fastq`.  Note they should be the same.
...
```{bash}
gzip R1_all_via_gzcat.fq | shasum 
